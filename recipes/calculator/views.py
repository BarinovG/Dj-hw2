...
def recipe_view(request, recipe_name):
    if recipe_name not in DATA:
        raise Http404(f'Рецепта {recipe_name} не найден')

    serving_count = int(request.GET.get('servings', 1))
    recipe = {'name': recipe_name, 'composition': deepcopy(DATA[recipe_name])}

    for ingridient in recipe['composition']:
        recipe['composition'][ingridient] *= serving_count
